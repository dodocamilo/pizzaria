<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>

<c:import url="/WEB-INF/jsp/header.jsp"/>

<div class="page-header">
	<h1>Bebida</h1>
</div>

<div class="col-md-6">
	<a href="${linkTo[PedidoController].categoria()}">
		<img data-holder-rendered="true" src="data:image/svg+xml;base64," 
			style="width: 80%; height: 80%;" data-src="holder.js/200x200" class="img-thumbnail" alt="80%x80%">
	</a>
</div>
<div class="col-md-6">
	<a href="${linkTo[PedidoController].categoria()}">
		<img data-holder-rendered="true" src="data:image/svg+xml;base64," 
			style="width: 80%; height: 80%;" data-src="holder.js/200x200" class="img-thumbnail" alt="200x200">
	</a>
</div>
<c:import url="/WEB-INF/jsp/footer.jsp"/>